...
const page = async ({ params }: { params: { email: string } }) => {
  ...
  try {
    ...
    if (email)
      resp = await axios.get(
        `https://mentor-match-smoky.vercel.app/api/get-user/${decodeURIComponent(
          email
        )}`
      );
    data = resp?.data;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center p-4 motion-preset-blur">
      <Card className="w-full max-w-3xl bg-white shadow-xl overflow-hidden">
